...
from backend.models.schemas.task_schema import TaskCreate, TaskResponse, TaskUpdate
from backend.services import task_service
from backend.services.auth_service import get_current_user

router = APIRouter(
    prefix="/targets/{target_id}/tasks",  
    tags=["Tasks"], 
)

@router.post("/", response_model=TaskResponse)
async def create_task(task: TaskCreate, db: AsyncSession = Depends(get_db), user_id: int = Depends(get_current_user)):
...
async def update_task(target_id: int, task_id: int, new_task: TaskUpdate, db: AsyncSession = Depends(get_db), user_id: int = Depends(get_current_user)):
    task = await task_service.update_task(db, new_task, task_id, target_id, user_id)
    if not task:
        raise HTTPException(status_code=404, detail="Task not found")
    return task